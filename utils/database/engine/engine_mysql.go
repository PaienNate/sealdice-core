...
func (s *MYSQLEngine) GetDataDB(_ consts.DBMode) *gorm.DB {
	return s.dataDB
}

func (s *MYSQLEngine) GetLogDB(_ consts.DBMode) *gorm.DB {
	return s.logsDB
}

func (s *MYSQLEngine) GetCensorDB(_ consts.DBMode) *gorm.DB {
	return s.censorDB
}
...
func (s *MYSQLEngine) Init(ctx context.Context) error {
	...
	var err error
	s.DB, err = backend.MySQLDBInit(s.DSN)
	if err != nil {
		return err
	}
