...
func (s *SQLiteEngine) getDBByModeAndKey(mode consts.DBMode, key dbName) *gorm.DB {
	// 取读者锁，从而允许同时获取大量的DB
	s.mu.RLock()
	defer s.mu.RUnlock()
	switch mode {
...
func (s *SQLiteEngine) GetDataDB(mode consts.DBMode) *gorm.DB {
	return s.getDBByModeAndKey(mode, DataDBKey)
}

func (s *SQLiteEngine) GetLogDB(mode consts.DBMode) *gorm.DB {
	return s.getDBByModeAndKey(mode, LogsDBKey)
}

func (s *SQLiteEngine) GetCensorDB(mode consts.DBMode) *gorm.DB {
...
func (s *SQLiteEngine) DBCheck() {
	...
	dataDB, err = backend.SQLiteDBInit(dbDataPath, false)
	if err != nil {
	...
	logsDB, err = backend.SQLiteDBInit(dbDataLogsPath, false)
	if err != nil {
	...
	censorDB, err = backend.SQLiteDBInit(dbDataCensorPath, false)
	if err != nil {
...
func (s *SQLiteEngine) dataDBInit() error {
	...
	readDB, writeDB, err := backend.SQLiteDBRWInit(dbDataPath)
	if err != nil {
		return err
	}
	// 添加并设置context
...
func (s *SQLiteEngine) LogDBInit() error {
	...
	readDB, writeDB, err := backend.SQLiteDBRWInit(dbDataLogsPath)
	if err != nil {
		return err
	}
	// 添加并设置context
...
func (s *SQLiteEngine) CensorDBInit() error {
	...
	readDB, writeDB, err := backend.SQLiteDBRWInit(path)
	if err != nil {
		return err
	}
	// 添加并设置context
