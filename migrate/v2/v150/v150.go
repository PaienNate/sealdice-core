...
`,
	ShouldRun: func(operator engine.DatabaseOperator) (bool, error) {
		...
		return db.Migrator().HasTable("attrs_user") ||
			db.Migrator().HasTable("attrs_group_user") ||
			db.Migrator().HasTable("attrs_group"), nil
	},
...
`,
	ShouldRun: func(operator engine.DatabaseOperator) (bool, error) {
		db := operator.GetDataDB(constant.READ)
		if !db.Migrator().HasTable("group_info") {
			return false, nil
		}
		var count int64
		err := db.Raw("SELECT count(*) FROM `sqlite_master` WHERE tbl_name = 'group_info' AND `sql` LIKE '%这个方法太严格了%'").Count(&count).Error
		if err != nil {
			return false, nil
		}
		return count > 0, nil
