...
// 添加一个敏感词记录
func CensorAppend(operator database.DatabaseOperator, msgType string, userID string, groupID string, content string, sensitiveWords interface{}, highestLevel int) bool {
	db := operator.GetCensorDB(consts.WRITE)
	// 获取当前时间的 Unix 时间戳
...
func CensorCount(operator database.DatabaseOperator, userID string) map[censor.Level]int {
	db := operator.GetCensorDB(consts.READ)
	// 定义要查询的不同敏感级别
...
func CensorClearLevelCount(operator database.DatabaseOperator, userID string, level censor.Level) {
	db := operator.GetCensorDB(consts.WRITE)
	// 使用 GORM 的链式查询执行批量更新
...
func CensorGetLogPage(operator database.DatabaseOperator, params QueryCensorLog) (int64, []model.CensorLog, error) {
	db := operator.GetCensorDB(consts.READ)
	var total int64
