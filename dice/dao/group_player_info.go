...
import (
	...
	"sealdice-core/utils/kratos"
)

// GroupPlayerNumGet 获取指定群组的玩家数量
func GroupPlayerNumGet(operator database.DatabaseOperator, groupID string) (int64, error) {
	// 使用读数据库
...
func GroupPlayerInfoGet(operator database.DatabaseOperator, groupID string, playerID string) *model.GroupPlayerInfoBase {
	// 使用读数据库
	db := operator.GetDataDB(consts.READ)
...
func GroupPlayerInfoSave(operator database.DatabaseOperator, info *model.GroupPlayerInfoBase) error {
	// 使用读数据库
