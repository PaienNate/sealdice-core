...
func LogGetInfo(operator database.DatabaseOperator) ([]int, error) {
	db := operator.GetLogDB(consts.READ)
	lst := []int{0, 0, 0, 0}
...
func LogGetLogs(operator database.DatabaseOperator) ([]*model.LogInfo, error) {
	db := operator.GetLogDB(consts.READ)
...
// LogGetLogPage 获取分页
func LogGetLogPage(operator database.DatabaseOperator, param *QueryLogPage) (int, []*model.LogInfo, error) {
	db := operator.GetLogDB(consts.READ)
	var lst []*model.LogInfo
...
// LogGetList 获取列表
func LogGetList(operator database.DatabaseOperator, groupID string) ([]string, error) {
	db := operator.GetLogDB(consts.READ)
	var lst []string
...
func LogGetUploadInfo(operator database.DatabaseOperator, groupID string, logName string) (url string, uploadTime, updateTime int64, err error) {
	db := operator.GetLogDB(consts.READ)
	var logInfo struct {
...
func LogSetUploadInfo(operator database.DatabaseOperator, groupID string, logName string, url string) error {
	db := operator.GetLogDB(consts.WRITE)
	if len(url) == 0 {
		return nil
	}
...
func LogGetAllLines(operator database.DatabaseOperator, groupID string, logName string) ([]*model.LogOneItem, error) {
	db := operator.GetLogDB(consts.READ)
	// 获取log的ID
...
func LogGetLinePage(operator database.DatabaseOperator, param *QueryLogLinePage) ([]*model.LogOneItem, error) {
	db := operator.GetLogDB(consts.READ)
	// 获取log的ID
...
func LogLinesCountGet(operator database.DatabaseOperator, groupID string, logName string) (int64, bool) {
	db := operator.GetLogDB(consts.READ)
	// 获取日志 ID
	logID, err := getIDByGroupIDAndName(db, groupID, logName)
	if err != nil || logID == 0 {
...
// LogDelete 删除log
func LogDelete(operator database.DatabaseOperator, groupID string, logName string) bool {
	db := operator.GetLogDB(consts.WRITE)
	// 获取 log ID
	logID, err := getIDByGroupIDAndName(db, groupID, logName)
	if err != nil || logID == 0 {
...
func LogAppend(operator database.DatabaseOperator, groupID string, logName string, logItem *model.LogOneItem) bool {
	db := operator.GetLogDB(consts.WRITE)
	// 获取 log ID
	logID, err := getIDByGroupIDAndName(db, groupID, logName)
	if err != nil {
...
func LogMarkDeleteByMsgID(operator database.DatabaseOperator, groupID string, logName string, rawID interface{}) error {
	db := operator.GetLogDB(consts.WRITE)
	// 获取 log id
	logID, err := getIDByGroupIDAndName(db, groupID, logName)
	if err != nil {
...
// LogEditByMsgID 编辑日志
func LogEditByMsgID(operator database.DatabaseOperator, groupID, logName, newContent string, rawID interface{}) error {
	db := operator.GetLogDB(consts.WRITE)
	logID, err := getIDByGroupIDAndName(db, groupID, logName)
	if err != nil {
		return err
